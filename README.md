...
| Command | Purpose |
|---|---|
| `/pr-review-loop` | Finds every open PR that needs your attention — never reviewed, or with new commits/comments since your last review — and loops through them one at a time: pulls PR description/discussion from `gh`, checks out the branch, sizes up the diff, then asks you for the ticket's title/description and which depth to review at. Runs `/review-quick` by default, or the full multi-agent `/review` when the PR earns it. Pass `full` (`/pr-review-loop full`) to default every PR to the deep pass. |
...
| `/review-pr` | Thorough multi-agent review of an open PR — up to six parallel agents (bugs, regressions, quality, risk, test coverage, and ticket/discussion alignment), scaled down when a diff doesn't warrant all of them. Takes ticket context inline from `/pr-review-loop` or falls back to `ticket.md`, and reads lint/test/build results from the PR's CI rather than running them locally. Slower and more expensive than `/review-quick` — use it for anything genuinely high-risk or unusually large, not for triaging a queue. |
