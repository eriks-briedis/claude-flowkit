...
- `ticket.md` is read by `/review` (it's your own ticket, that's the normal case) and, as a fallback, by `/review-pr`. Neither ever writes it. Inside `/pr-review-loop` it isn't touched at all: the ticket title/description is resolved per PR and passed inline, so a loop run can't overwrite a `ticket.md` holding your own in-progress work.
- `tickets/sprint.csv` is the fallback beneath `ticket.md`, and the only ticket source `/pr-review-loop` reads. Drop a Jira CSV export there and the commands stop asking you to paste ticket text they can look up: they pull the key out of the PR title or branch name and read the row for it. Column names are matched dynamically — a Jira export's `Issue key` / `Summary` / `Description` / `Custom field (Acceptance Criteria)` works, and so does a hand-written `id,title,description` file. Acceptance criteria, when present, reach the alignment agent as their own field rather than buried in the description. Everything degrades quietly: no file, no key in the file, or a CSV that won't parse just puts you back to being asked. Read-only, always — no command writes to it, including to correct a stale row. The commands name the source and key they resolved, so a stale export is visible rather than silently believed.
